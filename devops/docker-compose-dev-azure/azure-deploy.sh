...
resourcename="chending"  # lower case, only
resourcegroup="chending_rg"
...
# git clone https://github.com/apilogicserver/chending.git
# cd chending
...
az group create --name chending_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group chending_rg --sku S1 --is-linux
az container create --resource-group chending_rg --name chending --image apilogicserver/chending:latest --dns-name-label chending --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//chending.westus.azurecontainer.io:5656'
